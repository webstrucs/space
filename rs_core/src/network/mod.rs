...
async fn run_worker(worker_id: usize, listener: TcpListener, acceptor: TlsAcceptor, mut shutdown_rx: broadcast::Receiver<()>, connections: ConnectionMap, id_counter: Arc<AtomicUsize>) {
    loop {
        tokio::select! {
            biased;
            _ = shutdown_rx.recv() => {
...
async fn handle_connection(mut tls_stream: TlsStream<TcpStream>, addr: SocketAddr, connections: ConnectionMap, id_counter: Arc<AtomicUsize>) {
            ...
            if request_bytes.windows(3).any(|w| w == b"../") || request_bytes.windows(3).any(|w| w == b"..\\") {
                ...
                if let Ok(response_from_python) = send_to_python_and_get_response(conn_id, addr, request_data).await {
                   if let Err(e) = tls_stream.write_all(&response_from_python).await {
                    ...
                    let response = b"HTTP/1.1 502 Bad Gateway\r\n\r\nError communicating with application layer.";
                    if let Err(e) = tls_stream.write_all(response).await {
                         warn!(error = %e, "Erro ao escrever resposta 502 Bad Gateway");
                    }
                }
            }
        }
    ...
    if let Err(e) = tls_stream.shutdown().await { warn!(error = %e, "Erro no shutdown do socket"); }
